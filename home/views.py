...
def transactEth(request,amount,private_key):
    try:
        myuser=User.objects.get(email=request.user.email)
        profile=Customer.objects.get(user=myuser)
        account_1=profile.meta_key
    except:
        messages.error(request, 'Error - Internal Error cant fetch email.')
        return redirect('pricing')
    
    if(profile.is_connected == 'False'):
        messages.error(request, 'Error - Wallet Now Connected.')
        return redirect('pricing')
    try:
        alchemy_url = os.environ.get('ALCHEMY_URL')

        web3 = Web3(Web3.HTTPProvider(alchemy_url))    
        account_2 = "0x5bD8E749C554531359B0668f17F1Be6790F803E7"
        nonce = web3.eth.get_transaction_count(
            Web3.to_checksum_address(account_1))
        tx = {
            'to': account_2,
            'nonce': nonce,
    ...
    if request.user.is_authenticated:
        if request.method == 'POST': 
            ...
            if(ans == 0):
                ...
                return redirect('pricing')
            messages.success(request, 'Success - Transaction Successfull')
            return redirect('home')
        return render(request,"home/basic_pricing.html", context={}) 
...
def ispremium(request):
    if request.user.is_authenticated: 
        if request.method == 'POST':
            ...
            messages.success(request, 'Success - Transaction Successfull')
            return redirect('home')
        return render(request,"home/premium_pricing.html", context={}) 
    return render(request,"home/home.html", context={})  
...
def Add_key(request):
    if request.user.is_authenticated: 
        try:
            myuser=User.objects.get(email=request.user.email)
            profile=Customer.objects.get(user=myuser)
            profile.meta_key=request.GET.get('key')
            profile.is_connected=True
            profile.save()
            messages.success(request, 'Success - Wallet Connected Successfully.')
            return redirect('pricing')
        except:
            messages.error(request, 'Error - Internal Error cant fetch email.')
            return redirect('pricing')
